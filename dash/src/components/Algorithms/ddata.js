...
const questionsData = {
  ...
  "concepts":[
    ...
    {
      "title": "(question - Implement pow(x, n)) Approach / Algorithm Used",
      "answer": "APPROACH / ALGORITHM USED:\n- The task is to compute x raised to the power n (xⁿ) efficiently.\n- This is solved using **divide and conquer (binary exponentiation)**.\n- Base case: if n == 0, return 1 (any number to the power 0 is 1)\n- If n is negative:\n  - Convert x to 1/x and n to -n\n  - For edge case when n == Integer.MIN_VALUE (can't negate), handle separately:\n    → return 1 / (myPow(x, Integer.MAX_VALUE) * x)\n- Recursively compute half = myPow(x, n / 2)\n- Then square it: half * half\n- If n is odd, multiply once more by x\n\nTIME & SPACE COMPLEXITY:\nTime Complexity: O(log n)\n- Because the power is halved in every recursive call\nSpace Complexity: O(log n)\n- Due to recursive call stack\n\nDRY RUN / EXAMPLE:\nInput: x = 2.0, n = 5\n\nStep 1: n = 5 → not negative → continue\nStep 2: half = myPow(2.0, 2)\n  - myPow(2.0, 2):\n    → half = myPow(2.0, 1)\n      - myPow(2.0, 1):\n        → half = myPow(2.0, 0) = 1\n        → odd → return 2.0 * 1 = 2.0\n    → even → return 2.0 * 2.0 = 4.0\nStep 3: n = 5 is odd → return 2.0 * 4.0 = 8.0\n\n✔️ Final Output: 32.0 (2^5 = 32)\n\nEDGE CASE:\nInput: x = 2.0, n = -2147483648 (Integer.MIN_VALUE)\n→ Java can't negate Integer.MIN_VALUE, so we do:\n  1 / (myPow(2.0, 2147483647) * 2.0)\n\nOPTIMALITY NOTE:\n- This is the optimal method: O(log n) time and space\n- Compared to naive O(n), it's exponentially faster for large powers"
    },    
    {
